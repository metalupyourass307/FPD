...
// MODIFIED AGAIN to output the time evolution of each colloid's charge.
...
FILE *frho;
FILE *fvel;
FILE *fcharge; // ADDED: File for charge evolution
const char *filename;
...
void write_rho();
void write_vel();
void write_charge_evolution(); // ADDED: Prototype for new function
...
// ADDED: New function to write charge evolution
void write_charge_evolution() {
    // Assuming N=2, with colloid 0 being acid (plabel 1) and colloid 1 being base (plabel 2)
    if (N == 2) {
        fprintf(fcharge, "%d %lf %lf\n", int(t / DT), Z[0], Z[1]);
    }
}
...
#pragma omp parallel for
  ...
  if(int(t/DT)%interval_pos==0){
    charge();
    write_data();
    write_charge_evolution(); // ADDED: Call the new write function
    fflush(fw);
    fflush(fcharge); // ADDED: Flush the new file
  }
...
int main(int argc, char **argv){
  ...
  fcharge=fopen("charge_evolution.dat","w"); // ADDED: Open the new file
  register int i;
#ifdef _OPENMP
...
#pragma omp parallel for
  ...
  fftw_free(fftw_in);
  fftw_free(fftw_out);
  fflush(fw);fflush(fp);fflush(frho);fflush(fcharge); // ADDED: Final flush
  fclose(fw);fclose(fp);fclose(frho);fclose(fcharge); // ADDED: Close the new file
  return 0;
}