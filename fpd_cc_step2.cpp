// fpd_cc_ph7.cpp
// MODIFIED to run a constant charge simulation with charge values
// derived from the charge-regulation model at pH = 7.
...
void update_shape_function(){
  //update shape function and charge density here
  register int n,i;
#pragma omp parallel for
  for(i=1;i<=Nx;i++){
    for(int j=1;j<=Ny;j++){
...
#pragma omp parallel for reduction(+:pvolc,pvolx,pvoly,pvolz,pvolxy,pvolyz,pvolzx,dpvolc,dpvolx,dpvoly,dpvolz)
    for(i=0;i<RD2;i++){
      ...
      for(int j=0;j<RD2;j++){
        ...
        for(int k=0;k<RD2;k++){
          ...
          double dist2=drx1_2+dry1_2+drz1_2;
          
          //tanh
          if(dist2<radi2){
            double dist=sqrt(dist2)-radi;
            double phi1=phi_tanh(dist);
            double dphi1=dphi_tanh(dist);
            phic[n][i][j][k]=phi1;
            dphic[n][i][j][k]=dphi1;
            pvolc+=phi1;
            dpvolc+=fabs(dphi1);
  ...
  //field variable
  for(n=0;n<N;n++){
    double Ncn=fabs(Nc[n]);
    double coef1=Ncn*invpbvc[n];
    double rxn=rx[n];
    double ryn=ry[n];
    double rzn=rz[n];
    int RX=(int)rxn;
      ...
      for(i=0;i<RD2;i++){
        ...
        for(int j=0;j<RD2;j++){
          ...
          for(int k=0;k<RD2;k++){
            ...
            cp[ii][jj][kk]+=coef1*fabs(dphic[n][i][j][k]);
      ...
      for(i=0;i<RD2;i++){
        ...
        for(int j=0;j<RD2;j++){
          ...
          for(int k=0;k<RD2;k++){
            int kk=RZ+k-RD;
            if(kk<=0){kk+=Nz;}else if(kk>Nz){kk-=Nz;}
            cp2[ii][jj][kk]+=coef1*fabs(dphic[n][i][j][k]);
            phit[ii][jj][kk]+=phic[n][i][j][k];
            phixyt[ii][jj][kk]+=phixy[n][i][j][k];
            phiyzt[ii][jj][kk]+=phiyz[n][i][j][k];
            phizxt[ii][jj][kk]+=phizx[n][i][j][k];
...
void update_electric_equilibrium(){
  ...
  int ite_max=10000;
  double ethre=1.0e-4;
  double sor_coef=1.0;//determined empirically
...
#pragma omp parallel for reduction(+:cpt,cct,cp2t,cc2t,cspt,csmt)
    ...
    double normcc=cpt/cct;
    double normcc2=cp2t/cc2t;
    double normcsp=(n0*Volume)/cspt;
    double normcsm=(n0*Volume)/csmt;
#pragma omp parallel for
    for(i=1;i<=Nx;i++){
      for(int j=1;j<=Ny;j++){
        for(int k=1;k<=Nz;k++){
...
bool find_charge_from_csv(const std::string& filename, double target_pH, double& found_charge) {
  ...
  if (!std::getline(file, line)) {
      std::cerr << "ERROR: CSV file is empty or unreadable: " << filename << std::endl;
      return false;
  }
  ...
  while (std::getline(file, line)) {
      ...
      double current_pH = std::stod(cell);

      // Read Q
      if (!std::getline(ss, cell, ',')) continue;
      double current_Q = std::stod(cell);
      
      double diff = std::fabs(current_pH - target_pH);
      if (diff < min_diff) {
          min_diff = diff;
          best_q = current_Q;
          found = true;
  ...
  if (found) {
      ...
      std::cerr << "ERROR: No data found in CSV file: " << filename << std::endl;
      return false;
  }
}

void initial(int, double charge_acid, double charge_base){

  fr=fopen(filename,"r"); 
  double dummy_charge_from_file; 
  for(int l=0;l<N;l++) {
    fscanf(fr,"%d %lf %lf %lf %lf",&plabel[l],&rx[l],&ry[l],&rz[l], &dummy_charge_from_file);
    
    // Set charge based on plabel and the values found from the CSV files
    if (plabel[l] == 1) { // Acidic colloid
        Nc[l] = fabs(charge_acid);
    } else { // Basic colloid (plabel == 2)
        Nc[l] = fabs(charge_base);
    }
  }
  fclose (fr);

  // Add an informational printout to confirm the mode of operation.
  std::cout << "INFO: Running CONSTANT CHARGE (CC) simulation." << std::endl;
  // ==================== FIX 2: REMOVE OLD `cout` LINE ====================
  // The line below was removed because `charge_magnitude_at_pH7` no longer exists in this function.
  // std::cout << "INFO: Colloid charge magnitudes are FIXED to " << charge_magnitude_at_pH7 << " (derived from CR model at pH=7)." << std::endl;
  // ======================================================================
  std::cout << "INFO: Charge values are set based on the provided target pH." << std::endl;
  std::cout << "INFO: Charge values in the input file '" << filename << "' are IGNORED." << std::endl;
  std::cout << "---------------------------------------------------------" << std::endl;
  ...
  for(int i=0;i<N;i++){
    ...
    for(int j=i+1;j<N;j++){
      ...
      if(min>r_ij){min=r_ij;}
      if(r_ij<sig+2.0)
        std::cout << "warning: distance too short (may lead to bad dynamics)" << std::endl;
    }
  }
  ...
  if(abs(temp1+temp2)>0.0001)
    std::cout << "warning: colloids have net charges" << std::endl;
  for(int i=1;i<=Nx;i++){
    for(int j=1;j<=Ny;j++){
      for(int k=1;k<=Nz;k++){
        cc[i][j][k]=abs(temp1)/(invVol); //initial value
        cc2[i][j][k]=abs(temp2)/(invVol); //initial value
        csp[i][j][k]=n0;
        csm[i][j][k]=n0;
      }
    }
...
int main(int argc, char **argv){
  ...
  // MODIFIED: Now expects 7 arguments, including the target pH
  if(argc!=7){
    std::cout << "Usage: " << argv[0] <<" [init state] [time] [beta] [eps lj6] [c_salt] [target_pH]" << std::endl;
    exit(0);
  } 
  ...
  scr=atof(argv[4]);
  n0=atof(argv[5]);
  double target_pH = atof(argv[6]); // New parameter for target pH

  // ===================== NEW LOGIC START =====================
  double charge_acid, charge_base;
  if (!find_charge_from_csv("step1_weak_acid_Q_vs_pH.csv", target_pH, charge_acid)) {
      return 1; // Exit if file reading fails
  }
  if (!find_charge_from_csv("step1_weak_base_Q_vs_pH.csv", target_pH, charge_base)) {
      return 1; // Exit if file reading fails
  }
  // ===================== NEW LOGIC END =======================
  ...
  std::cout << "---------------------------------------------------------" << std::endl;
  std::cout << "INFO: Running CONSTANT CHARGE (CC) simulation." << std::endl;
  std::cout << "INFO: Target pH = " << target_pH << std::endl;
  std::cout << "INFO: Looked up charges from CSV files:" << std::endl;
  std::cout << "      - Acid (plabel 1) Charge: " << charge_acid << std::endl;
  std::cout << "      - Base (plabel 2) Charge: " << charge_base << std::endl;
...
#pragma omp parallel for
  ...
  // MODIFIED: Pass the found charges to the initial function
  initial(0, charge_acid, charge_base);
