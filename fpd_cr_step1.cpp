...
/*************** STEP1: pH sweep config (enabled when compiling with -DSTEP1_SWEEP) ***************/
#ifdef STEP1_SWEEP
static const double STEP1_QMAX = 100.0;
static const double STEP1_PH_MIN = 2.0;
static const double STEP1_PH_MAX = 12.0;
static const double STEP1_PH_DPH = 0.2;
static const double STEP1_pKa = 4.5;
static const double STEP1_pKb = 4.5;
static const char* STEP1_CSV_ACID = "step1_weak_acid_Q_vs_pH.csv";
static const char* STEP1_CSV_BASE = "step1_weak_base_Q_vs_pH.csv";
#endif
...
#define Ncp2    0 //number of positive colloids type 2
...
/*********** CR Model Parameters ************/
double pKa = 4.5;       // ADD: pKa of surface groups (can be read from argv)
double pH = 7.0;        // ADD: pH of the bulk solution (can be read from argv)
double K_const;         // ADD: Helper constant 10^(pH-pKa)
    ...
    for (int i = 0; i < RD2; ++i) {
      ...
      for (int j = 0; j < RD2; ++j) {
        ...
        for (int k = 0; k < RD2; ++k) {
          ...
          double alpha_cell;
          if (plabel[n] == 1) {
            const double X = K_const * exp(-Uijk);  // acid
            alpha_cell = X / (1.0 + X);
          } else {
            const double X = K_const * exp(+Uijk);  // base
            alpha_cell = X / (1.0 + X);
          }
    ...
    for(i=1;i<=Nx;i++){
        for(int j=1;j<=Ny;j++){
            for(int k=1;k<=Nz;k++){
                ...
                // For acidic groups (plabel 1 -> negative charge), exponent is +U
                // Formula: alpha = X / (1 + X) where X = K_const * exp(+U)
                double X_acid = K_const * exp(Uijk);
                double alpha_acid = X_acid / (1.0 + X_acid);
                cp[i][j][k] = cp_max[i][j][k] * alpha_acid;
                
                // For basic groups (plabel 2 -> positive charge), exponent is -U
                // Formula: alpha = X / (1 + X) where X = K_const * exp(-U)
                double X_base = K_const * exp(-Uijk);
...
void initial(int){
  ...
  fclose (fr);
    ...
    double initial_alpha = K_const / (1.0 + K_const);
    double total_charge_guess1 = 0.0; 
    double total_charge_guess2 = 0.0; 
  
    for(int l=0; l<N; ++l) {
      if(plabel[l] == 1) {
          total_charge_guess1 -= N_sites[l] * initial_alpha;
      } else {

          total_charge_guess2 += N_sites[l] * initial_alpha; 
      }
    }
...
int main_normal(int argc, char **argv){
  ...
  scr=atof(argv[4]);
  n0=atof(argv[5]);
  pKa=atof(argv[6]); // ADDED: Read pKa from the 6th argument
  pH=atof(argv[7]);  // ADDED: Read pH from the 7th argument
  // Check for the correct number of arguments (program name + 7 parameters = 8)
  if(argc!=8){ // MODIFIED: Check for 8 arguments instead of 6
    // MODIFIED: Update usage message to include pKa and pH
    std::cout << argv[0] <<" [init state] [time range] [beta] [eps lj6] [c_salt] [pKa] [pH]" << std::endl;
    exit(0);
  } 

  else{
    // output setting parameters
    K_const = pow(10.0, pH - pKa);
    ...
    std::cout << "pKa=" << pKa << ", pH=" << pH << std::endl;
...
/*************** Charge-balance checker (sum over grid) ***************/
static inline void log_charge_balance_once(const char* tag) {
#ifdef STEP1_SWEEP
  double Qtot=0.0, Qcp=0.0, Qcc=0.0, Qcp2=0.0, Qcc2=0.0, Qspp=0.0, Qsmm=0.0;
  #pragma omp parallel for reduction(+:Qtot,Qcp,Qcc,Qcp2,Qcc2,Qspp,Qsmm)
  for (int i=1;i<=Nx;i++){
    for (int j=1;j<=Ny;j++){
      for (int k=1;k<=Nz;k++){
        Qcp  +=  cp[i][j][k];
        ...
        Qspp +=  csp[i][j][k];
        Qsmm +=  csm[i][j][k];
        Qtot += -cp[i][j][k] + cc[i][j][k] + cp2[i][j][k] - cc2[i][j][k] + csp[i][j][k] - csm[i][j][k];
      }
    }
  }
  printf("[charge-balance %s] (cc-cp)=%.3e, (cc2-cp2)=%.3e, (csp-csm)=%.3e, Q_total=%.3e\n",
         tag, Qcc-Qcp, Qcc2-Qcp2, Qspp-Qsmm, Qtot);
#else
  (void)tag;
#endif
}
...
#ifdef STEP1_SWEEP
static void reset_all_fields_to_zero() {
  #pragma omp parallel for
  for (int i=1;i<=Nx;i++) for (int j=1;j<=Ny;j++) for (int k=1;k<=Nz;k++) {
    cp[i][j][k]=cc[i][j][k]=cp2[i][j][k]=cc2[i][j][k]=csp[i][j][k]=csm[i][j][k]=0.0;
    rho[i][j][k]=U[i][j][k]=0.0;
  }
}
static void place_single_colloid(int type_label, double Nsites_abs) {
  plabel[0] = type_label; // 1: acid(-), 2: base(+)
  rx[0]=(Nx+1)/2; ry[0]=(Ny+1)/2; rz[0]=(Nz+1)/2;
  N_sites[0] = Nsites_abs;
  update_shape_function();
  update_max_charge_density();
}
static double run_one_pH(double pH_value, double pK_surface) {
  // K_const = pow(10.0, pH_value - pK_surface); // OLD INCORRECT CODE

  // NEW CORRECTED CODE
  // Check if the colloid is an acid (label 1) or a base (label 2)
  if (plabel[0] == 1) { // It's an acid
      K_const = pow(10.0, pH_value - pK_surface);
  } else { // It's a base
      double pOH = 14.0 - pH_value;
      K_const = pow(10.0, pOH - pK_surface);
  }

  reset_all_fields_to_zero();
  initial(0);
  update_electric_equilibrium();
  update_colloid_Nc_from_alpha();
  charge();
  return Z[0];
}
int main(int argc, char** argv) {
  FILE* fa = fopen(STEP1_CSV_ACID, "w"); fprintf(fa, "pH,Q\n");
  FILE* fb = fopen(STEP1_CSV_BASE, "w"); fprintf(fb, "pH,Q\n");

  place_single_colloid(1, STEP1_QMAX);
  for (double pH = STEP1_PH_MIN; pH <= STEP1_PH_MAX + 1e-12; pH += STEP1_PH_DPH) {
    double Q = run_one_pH(pH, STEP1_pKa);
    fprintf(fa, "%.6f, %.10f\n", pH, Q);
    log_charge_balance_once("acid");
  }

  place_single_colloid(2, STEP1_QMAX);
  for (double pH = STEP1_PH_MIN; pH <= STEP1_PH_MAX + 1e-12; pH += STEP1_PH_DPH) {
    double Q = run_one_pH(pH, STEP1_pKb);
    fprintf(fb, "%.6f, %.10f\n", pH, Q);
    log_charge_balance_once("base");
  }

  fclose(fa); fclose(fb);
  printf("STEP1 sweep done. Output: %s, %s\n", STEP1_CSV_ACID, STEP1_CSV_BASE);
  return 0;
}
#else
int main(int argc, char** argv) { return main_normal(argc, argv); }
#endif